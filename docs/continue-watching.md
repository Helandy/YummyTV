...
При выборе карточки `ResolveContinueWatchingLaunchUseCase` загружает список видео тайтла и
возвращает нейтральную playback-цель. Presentation преобразует ее в destination плеера.
