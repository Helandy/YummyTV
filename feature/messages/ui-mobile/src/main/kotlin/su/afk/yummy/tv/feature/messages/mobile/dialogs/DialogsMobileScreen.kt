...
fun DialogsMobileScreen(
    ...
    // Рядом с открытым чатом граф сужает панель списка: тогда вместо строк — колонка аватарок.
    BoxWithConstraints {
        val isCompact = MobileCompactListPane.isCompact(maxWidth)
        BaseScreen(
            isScroll = false,
            customTopBar = {
                MobileTopBar(
                    title = if (isCompact) "" else stringResource(R.string.messages_title),
                        ...
                        when {
                            ...
                            dialogs.itemCount == 0 -> item {
                                ...
                                dialogs[index]?.let { dialog ->
                                    ...
                                    if (isCompact) {
                                        DialogMobileAvatarItem(dialog, onClick)
                                    } else {
                                        DialogMobileRow(dialog, onClick)
                                    }
                                }
                            }
                        }
