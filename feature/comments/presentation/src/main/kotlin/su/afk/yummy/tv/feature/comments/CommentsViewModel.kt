...
class CommentsViewModel @AssistedInject internal constructor(
...
) : BaseViewModel<CommentsState.State, CommentsState.Event, CommentsState.Effect>() {
    ...
    private fun submit() {
        ...
        viewModelScope.launch {
            ...
            when (val mode = currentState.composerMode) {
                ComposerMode.New -> mutationHandler.create(
                    targetType,
                    targetId,
                    CommentDraft(text),
                ).onSuccess { created ->
                ...
                is ComposerMode.Reply -> mutationHandler.create(
                    targetType,
                    targetId,
                    CommentDraft(
                        text = text,
                ...
                is ComposerMode.Edit -> mutationHandler.update(
                    mode.commentId,
                    text,
                ).onSuccess { updated ->
    ...
    private fun confirmDelete() {
        ...
        viewModelScope.launch {
            setState { copy(isMutating = true) }
            mutationHandler.delete(comment.id).fold(
                onSuccess = { deleted ->
                    if (deleted) {
    ...
    private fun confirmReport(reason: CommentReportReason) {
        ...
        viewModelScope.launch {
            setState { copy(isMutating = true) }
            mutationHandler.report(comment.id, reason).fold(
                onSuccess = { reported ->
                    if (reported) {
    ...
    private fun vote(commentId: Int, vote: CommentVote) {
        ...
        val comment = findComment(commentId) ?: return
        viewModelScope.launch {
            val result = mutationHandler.changeVote(commentId, comment.vote, vote)
            result.fold(
                onSuccess = { change ->
                    val voteResult = change.result
                    val newVote = change.vote
